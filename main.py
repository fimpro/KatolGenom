...
def interfejs(okno):  # funkcja obsługująca główny interfejs programu(menu widoczne po włączeniu
    ...
    recznieButton = CTkButton(okno, text="Dane Ręcznie",
                              command=lambda: wczytaj_recznie(okno, lancpoczatkowy=""))
...
def rysuj_przyciski(ramka, i, bialko, przesuniecie):
    ...
    Button = CTkButton(ramka, text="G. szczegółowo", width=(size[0] / 15) * 2,
...
def przepisNaZmiane(obraz2, wybor, ramka): #funckja zwracająca event dzieki której morzemy po włączeniu instrukcji łatwiej robić cokolwiek po zmianie okienka w comboboxsie
    ...
    if wybor == "Wstep":  # podmieniam pliki zależnie od wyboru
        file = "wstep.PNG"
    elif (wybor == "Opcje"):
        file = "opcje.PNG"

    elif (wybor == "Dane Ręcznie"):

        file = "danerecznie.PNG"

    elif (wybor == "Dane z pliku"):
        file = "danezpliku.PNG"

    elif (wybor == "Szukaj białek"):
        file = "szukajbialek.PNG"

    elif (wybor == "Generowanie"):
    ...
    obraz.grid(row=0, column=0, rowspan=3, sticky=W+E+N+S)
...
def instrukcja(okno):  # funkcja wczytująca instruckje użytkowania
    ...
    img1 = CTkImage(light_image=Image.open("wstep.png"),
                    ...
                    size=(600, 800))
    obraz = CTkLabel(ramka, text="", image=img1)
    obraz.grid(row=0, column=0, rowspan=3, sticky=N+W+E+S)


    # tworze combobox
    def zmiana(choice):
        przepisNaZmiane(obraz, choice, ramka)
    wybor = StringVar(value="Wstep")
    instrukcja_cb = CTkOptionMenu(master=okno,
                                     values=["Wstep", "Opcje", "Dane Ręcznie", "Dane z pliku", "Szukaj białek", "Generowanie", "Wykresy"],command=zmiana)




    #odpalam funkcje która po zwrocie eventu bedzie zmieniałą obraz w zaleznosci od wyboru
    instrukcja_cb.grid(row=1, column=4, sticky=W + E)

    #tworze widgety
    napis = CTkLabel(okno, text="Instrukcja")
    napis.grid(row=0, column=0,columnspan=4, sticky=W + E)

    wrocButton = CTkButton(okno, text="Wroc", width=15, command=lambda: interfejs(okno))
    wrocButton.grid(row=0, column=4, sticky=W + E)
